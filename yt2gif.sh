...
ffmpeg -ss "$START" -t "$LENGTH" -i dl.mp4 -i palette.png -filter_complex "fps=15[x];[x][1:v]paletteuse" "$NAME"_slow.gif

# mv dl.gif "$NAME".gif
echo " ===> Speeding gif..."
ffmpeg -i "$NAME"_slow.gif -i palette.png -vf "setpts=(1/"$SPEED")*PTS" "$NAME".gif
echo " ===> Removing files..."
rm dl.mp4
rm palette.png
